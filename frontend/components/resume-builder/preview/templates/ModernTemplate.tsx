...
export default function ModernTemplate({ resume }: { resume: Resume }) {
            ...
            case "summary":
                return resume.personalInfo.summary ? (
                    <section key="summary" className="mb-4">
                        <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-900 border-b border-slate-300 mb-2 pb-1">Professional Summary</h2>
            ...
            case "skills":
                return resume.skills.length > 0 ? (
                    <section key="skills" className="mb-4">
                        <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-900 border-b border-slate-300 mb-2 pb-1">
                            Skills
                        </h2>
                        <div className="flex flex-wrap gap-2 text-sm">
                            {resume.skills.map((skill) => (
                                <span key={skill.id} className="font-medium text-slate-700 bg-slate-100 px-2 py-1 rounded">
                                    {skill.name} {skill.level !== "Intermediate" && <span className="text-slate-400 font-normal">({skill.level})</span>}
                                </span>
                            ))}
                        </div>
                    </section>
                        ...
                        <div className="space-y-3">
                            {resume.education.map((edu) => (
                                <div key={edu.id}>
                                    ...
                                    {edu.gpa && (
                                        <div className="text-sm text-slate-500 mt-0.5">GPA: {edu.gpa}</div>
                                    )}
                                </div>
                            ))}
                        </div>
                        ...
                        <ul className="list-disc list-inside text-sm text-slate-700 space-y-1">
                            {resume.achievements.map(ach => (
                                <li key={ach.id}>
                                    ...
                                    {ach.description && <span className="block ml-4 text-slate-500 mt-0.5">{ach.description}</span>}
                                </li>
                            ))}
                        </ul>
                    </section>
    ...
    return (
        <div className="w-full h-full bg-white text-slate-900 font-sans">
            <header className="mb-4 flex flex-col items-center border-b-[3px] border-slate-900 pb-4">
                <h1 className="text-3xl font-bold uppercase tracking-wide text-slate-900 text-center">
                    {resume.personalInfo.fullName || "YOUR NAME"}
                </h1>

                <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 mt-3 text-sm text-slate-600">
